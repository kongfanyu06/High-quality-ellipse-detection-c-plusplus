...
{
	std::cout << ("circleDetection Error: %s\n", msg);
	exit(EXIT_FAILURE);
}
...
int fitEllipse(point2d* dataxy, int datanum, double* ellipara) {
	// 构造 D 矩阵
	Eigen::MatrixXd D(datanum, 6);
	for (int i = 0; i < datanum; i++) {
		D(i, 0) = dataxy[i].x * dataxy[i].x;
		D(i, 1) = dataxy[i].x * dataxy[i].y;
		D(i, 2) = dataxy[i].y * dataxy[i].y;
		D(i, 3) = dataxy[i].x;
		D(i, 4) = dataxy[i].y;
		D(i, 5) = 1;
	}
	...
	if (info == 0)
	{
		...
		for (int i = 0; i < realPart.size(); i++)
		{
			...
			if (realPartValue >= -(2.2204460492503131e-14) && imagPart[i] == 0 && beta[i] != 0)
			{
				index = i;
				//break;
			}
		...
		if (index == -1)
		{
			...
			for (int i = 0; i < realPart.size(); i++)
			{
				...
				if (realPartValue >= temp && imagPart[i] == 0 && beta[i] != 0)
				{
					temp = realPartValue;
					index = i;
				}
...
int fitEllipse2(double* S, double* ellicoeff) {
	// 将输入的 S 转换为 Eigen 矩阵
	Eigen::MatrixXd S_mat = Eigen::Map<Eigen::Matrix<double, 6, 6, Eigen::RowMajor>>(S);
	...
	// 计算广义特征值问题：eig(S^-1 * C)
	Eigen::GeneralizedEigenSolver<Eigen::MatrixXd> solver(S_mat, C);
	...
	if (info == 0)
	{
		...
		for (int i = 0; i < realPart.size(); i++) {
			...
			/*if (-1 < (realPart[i] - 6135.68) && (realPart[i] - 6135.68) < 1)
			{
				cout << realPart.size() << " " << realPart[i] << " " << realPartValue <<" " << imagPart[i] << " " << beta[i] << " "
					<< (realPartValue >= -(2.2204460492503131e-14) && imagPart[i] == 0 && beta[i] != 0) << " "
					<< (realPartValue >= -(2.2204460492503131e-14)) << " " << (imagPart[i] == 0) << " " << (beta[i] != 0) << endl;
				for (int a = 0; a < 6; a++)
				{
					for (int b = 0; b < 6; b++)
					{
			...
			/*cout << realPart.size() << " " << realPart[i] << " " << imagPart[i] << " " << beta[i] << " "
				<< (realPart[i] >= -(2.2204460492503131e-14) && imagPart[i] == 0 && beta[i] != 0) << " "
				<< (realPart[i] >= -(2.2204460492503131e-14)) << " " << (imagPart[i] == 0) << " " << (beta[i] != 0) << endl;*/
			if (realPartValue >= -(2.2204460492503131e-14) && imagPart[i] == 0 && beta[i] != 0) {
				index = i;
				//break;
			}
		...
		if (index == -1) {
			...
			for (int i = 0; i < realPart.size(); i++) {
				...
				/*cout << realPart.size() << " " << realPart[i] << " " << imagPart[i] << " " << beta[i] << " "
					<< (realPart[i] >= temp && imagPart[i] == 0 && beta[i] != 0) << " "
					<< (realPart[i] >= temp) << " " << (imagPart[i] == 0) << " " << (beta[i] != 0) << endl;*/
				if (realPartValue >= temp && imagPart[i] == 0 && beta[i] != 0) {
					temp = realPartValue;
					index = i;
				}
...
void drawEllipse(Mat img, double* ellipara)
{
	...
	{
		ccc = 255;
		std::cout << cnt / 2 << '\t' << ellipara[0] << '\t' << ellipara[1] << "\t" << ellipara[2] << '\t' << ellipara[3] << '\t' << ellipara[4] << endl;
	}
	cnt++;
...
PairGroupList* getValidInitialEllipseSet(double* lines, int line_num, vector<vector<int>>* groups, double* coverages, image_double angles, double distance_tolerance, int specified_polarity)
{
	...
	free(dataxy);//释放内存
	//std::cout << "groupsNum" << groupsNum << std::endl;
	...
	for (i = 0; i < groupsNum; i++)
	{
		...
		{
			//cout << coverages[i] << " " << M_4_9_PI << endl;
			//加入极性判断,只提取指定极性的椭圆
...
void findEllipse(cv::Mat candidates_ellipse, int imgy, int imgx, uchar* inputimg, int edge_process_select, int specified_polarity, cv::Mat &imageEdge, cv::Mat& gradient_vec_out, cv::Mat &ls_mat)
{
	// cv::Mat matrix(rows, candidates_num, CV_64F, cv::Scalar(0));

	// 获取矩阵数据指针（类似 mxGetPr）
	//double* candidates_out = (double*)matrix.ptr<double>(0);
	...
	error("The number of output arc-support line segments: %i\n" + n);
	error("The number of arc-support groups:%i\n" + groups.size());
	/*int groups_t = 0;
	...
	else
	{
		...
		//candidates_out = (double*)mxGetPr(plhs[0]);
		candidates_ellipse = Mat(5, 1, CV_64F, cv::Scalar(0));
		candidates_out = (double*)candidates_ellipse.ptr<double>(0);
	 ...
	 for ( int c = 0; c < imgx; c++ )
		 for ( int r = 0; r < imgy; r++)
		 {
			 ...
			 if (angles->data[addr] == NOTDEF)
				 imageEdge.at<uchar>(c, r) = 0;
			 else
			 {
				 imageEdge.at<uchar>(c, r) = 255;
	 ...
	 gradient_vec_out = Mat::zeros(imgy, imgx, CV_64FC(2));
	  for ( int c = 0; c < imgx; c++ )
		 for ( int r = 0; r < imgy; r++)
		 {
			 ...
			 if(angles->data[addr] != NOTDEF)
			 {
				 gradient_vec_out.at<Vec2d>(r, c)[0] = cos(angles->data[addr]);
				 gradient_vec_out.at<Vec2d>(r, c)[0] = sin(angles->data[addr]);
			 }
		 }
	 //---------------------------------------------------------------------
	//输出线段检测的图像

	ls_mat = Mat::zeros(imgy,imgx,CV_8UC1);
...
void mexFunction1(const Mat& inputImg, int edge_process_select, int specified_polarity, vector<Ellipse>& candidates_out, cv::Mat& imageEdge, vector<Point2d>& normals, cv::Mat& ls_mat, vector<Point2d>& points)
{
	...
	// std::cout << edge_process_select << ":" << specified_polarity << std::endl;
	//int edge_process_select = (int)mxGetScalar(prhs[1]);//边缘提取选择，1 canny; 2 sobel
	//int specified_polarity = (int)mxGetScalar(prhs[2]);//1,指定检测的椭圆极性要为正; -1指定极性为负; 0表示两种极性椭圆都检测
	/*imgy = (int)mxGetM(prhs[0]);
	imgx = (int)mxGetN(prhs[0]);*/
	int imgy = inputImg.rows;
	int imgx = inputImg.cols;
	//double* data = (double*)malloc(imgy * imgx * sizeof(double));//将输入矩阵中的图像数据转存到一维数组中

	//cv::Mat image = imread("D:/code/c++/double_camera/demo/38L.png", CV_LOAD_IMAGE_GRAYSCALE);;
	// std::cout << edge_process_select << ":" << specified_polarity << std::endl;
	...
	/*for (int c = 0; c < imgx; c++)
	{
		for (int r = 0; r < imgy; r++)
		{
			...
			if (c < 5 && r < 5)
			{
				std::cout << data[c + r * imgx] << "," << inputimg[r + c * imgy] << std::endl;
			}
		}
	}*/
	// Convert input image to a double array
	double* data = new double[imgy * imgx];
	for (int r = 0; r < imgy; ++r) {
		for (int c = 0; c < imgx; ++c) {
			data[c + r * imgx] = static_cast<double>(inputImg.at<uchar>(r, c));
			/*if (c < 5 && r < 5)
			{
				std::cout << data[c + r * imgx] << "," << inputImg.at<uchar>(r, c) << std::endl;
			}*/
		}
	}
	int n;//线段数量
	...
	free(reg); //释放内存
	calcuGroupCoverage(out, n, groups, coverages);//计算每个组的覆盖角度
	//error("The number of output arc-support line segments");
	/*std::cout << imgy << "," << imgx << std::endl;;
	...
	int  candidates_num = 0;//候选椭圆数量
	//rejectShortLines(out,n,&new_n);
	//std::cout << out << n << coverages << angles << distance_tolerance << "," << specified_polarity << "," << groups.size() << std::endl;
	pairGroupList = getValidInitialEllipseSet(out, n, &groups, coverages, angles, distance_tolerance, specified_polarity);
	//error("1112");
	if (pairGroupList != NULL)
	{
		...
		for (int i = 0; i < candidates_num; i++)
		{
			candidates_out.push_back({ candidates[0 + i * 5] ,candidates[1 + i * 5] ,candidates[2 + i * 5] ,candidates[3 + i * 5] ,candidates[4 + i * 5] });
		}		//plhs[0] = mxCreateDoubleMatrix(5, candidates_num, mxREAL);
		//candidates_out = (double*)mxGetPr(plhs[0]);
		////候选圆组合(xi,yi,ai,bi,phi_i)', 5 x candidates_num, 复制到矩阵candidates_out中
		//memcpy(candidates_out, candidates, sizeof(double) * 5 * candidates_num);
	...
	else
	{
		...
		candidates_out.push_back({0, 0, 0, 0, 0});
	}
	uchar* edgeimg_out;
	unsigned long edge_pixels_total_num = 0;//边缘总像素
	//double *gradient_vec_out;
	...
	//将梯度向量存到矩阵gradient_vec_out中
	unsigned long addr, g_cnt = 0;
	imageEdge = Mat::zeros(imgy, imgx, CV_8UC1);
	for (int c = 0; c < imgx; c++)
		for (int r = 0; r < imgy; r++)
		{
			addr = r * imgx + c;
			if (angles->data[addr] == NOTDEF)
				imageEdge.at<uchar>(r, c) = 0;
			else
			{
				normals.push_back({ cos(angles->data[addr]), sin(angles->data[addr]) });
				imageEdge.at<uchar>(r, c) = 255;
				points.push_back({(double)c, (double)r});
				// edgeimg_out[c*imgy+r] = 255;//为边缘点，赋值为白色
				 //------------------------------------------------
				edge_pixels_total_num++;
			}
   ...
   //输出线段检测的图像
	...
	//{
	//	for (int i = 0; i < candidates_num; i++)
	//		ellipse(ls_mat, cv::Point((int)candidates_out.at(i).x, (int)candidates_out.at(i).y), cv::Size(candidates_out.at(i).a, candidates_out.at(i).b), candidates_out.at(i).phi * 180 / M_PI, 0, 360, (Scalar(255, 0, 0)), 1);
	//}
	//---------------------------------------------------------------------
	//这里的free是释放程序中用于产生候选圆所用到的一系列内存
	free(data);
